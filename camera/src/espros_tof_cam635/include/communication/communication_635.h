...
private:
    ...
    void processTemporalHdr(std::shared_ptr<ComLib::TofCam635Image> &image);
    void patchImage(std::shared_ptr<ComLib::TofCam635Image> &imageTarget, std::shared_ptr<ComLib::TofCam635Image> imageCompare);
    bool allImagesFilled(const unsigned int numImage);

    static const unsigned int NUM_HDR_IMAGE = 3;
    std::shared_ptr<ComLib::TofCam635Image> hdrImage[NUM_HDR_IMAGE];
    std::shared_ptr<ComLib::TofCam635Image> resultImage;
